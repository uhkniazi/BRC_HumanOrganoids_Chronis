...
parameters {
  ...
  real<lower=0> sigmaRan1; //[NScaleBatches1]; // random effect standard deviations for sub-batches in group 1
...
transformed parameters {
  ...
  mu = exp(betas + rGroupsJitter1[NgroupMap1]);// + rGroupsJitter2[NgroupMap2]*sigmaRan2);
}
model {
  ...
  // likelihood function
  y ~ neg_binomial(mu, iSize[NsizeMap]);
}
